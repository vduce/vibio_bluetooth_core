...
class VibioBluetoothCore {
  ...
  static Future<Map?> firstStepParams() async {
    final Map? firstStepMap =
    await _channel.invokeMethod('fhAuthFirstStepParams');
    return firstStepMap;
  }

  //secondStep
  // Parameters for the second step of obtaining certification
  static Future<Map?> secondStepParams(String params) async {
    final Map? secondStepMap = await _channel
        .invokeMethod('fhAuthSecondStepParams', {"params": params});
